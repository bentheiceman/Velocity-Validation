...
-- ========================================================================================================================================
-- NOTE (2026-02-04): Snowsight session variables (e.g., `set forecast_weeks=13;` then `$forecast_weeks`)
-- will throw "Session variable ... does not exist" if Step 1 isn't executed in the *same session*.
-- To make Step 2 runnable standalone, parameters are also defined in the `params` CTE below.
--
-- If you prefer session variables, you can uncomment these lines and update the `params` CTE to match.
...
create or replace table dm_supplychain.public.sku_velocity_reclassification_analysis_hds as (
    ...
    with 
    -- Parameters (inline defaults so Step 2 runs even if Step 1 wasn't executed)
    params as (
        select
    ...
    -- 13-week forecast window (set parameters)
    ...
    ,high_cube_items as  ( 
        ...
        where udc_product_cat_descr in (
            'Printing Supplies','Paper Product',
    ...
    -- UNIONING/JOINING ALL 3 GROUPS FOR FULL VELOCITY RECLASSIFICATION
